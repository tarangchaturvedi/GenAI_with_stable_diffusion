...
def test(opt, seg, gmm, alias):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            cm = (c.mean(dim=1, keepdim=True) > 0.5).float()  # Generate mask from clothing image
            cm = F.interpolate(cm, size=(c.shape[2], c.shape[3]), mode='bilinear', align_corners=False).cuda()
